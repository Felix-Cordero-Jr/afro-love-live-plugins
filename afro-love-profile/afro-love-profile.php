 ...
 * Description: Front-end profile creation + member browsing for Afro Love Life dating site (with fast lightbox + online presence ping).
 * Version: 1.2.2
...
/**
 * Hide WP admin bar on the front-end for all users (still visible in /wp-admin).
 */
...
/**
 * Keep online status accurate (frontend presence ping).
 * - Updates afl_last_active every 30s while user is browsing front-end pages.
 */
add_action( 'wp_enqueue_scripts', function () {
	if ( ! is_user_logged_in() || is_admin() ) return;

	wp_register_script( 'afl-presence', false, [ 'jquery' ], '1.0', true );
	wp_localize_script( 'afl-presence', 'aflPresence', [
...
(function($){
  function ping(){
    $.post(aflPresence.ajaxurl, { action:'afl_presence_ping', nonce: aflPresence.nonce });
  }
  ping();
...
add_action( 'wp_ajax_afl_presence_ping', function () {
...
/**
 * Enqueue Dashicons + Like/Favorite actions + Lightbox assets
 */
add_action( 'wp_enqueue_scripts', function () {
	...
	// Like/Fav script (optional; you already have this file)
	wp_register_script(
		'afl-actions',
	...
	wp_localize_script( 'afl-actions', 'aflAjax', [
		'ajaxurl' => admin_url( 'admin-ajax.php' ),
		'nonce'   => wp_create_nonce( 'afl_actions_nonce' ),
	] );

	// Lightbox (inline, no external libs needed)
...
/* Make images/buttons feel clickable */
...
(function(){
  ...
  document.addEventListener("click", function(e){
    ...
    var groupEls = qa("[data-afl-lightbox][data-afl-group=\\"" + groupKey + "\\"]");
...
/**
 * Toggle helper for likes/favorites
 */
function afl_toggle_user_list_meta( $current_user_id, $meta_key, $target_user_id ) {
	...
	if ( in_array( $target_user_id, $list, true ) ) {
		...
		return [ 'active' => false, 'list' => $list ];
	} else {
		$list[] = $target_user_id;
		$list   = array_values( array_unique( array_map( 'intval', $list ) ) );
		update_user_meta( $current_user_id, $meta_key, $list );
		return [ 'active' => true, 'list' => $list ];
	}
}
 ...
 * AJAX: Like
 */
add_action( 'wp_ajax_afl_toggle_like', function () {
	if ( ! is_user_logged_in() ) wp_send_json_error( [ 'msg' => 'Not logged in' ], 403 );
	check_ajax_referer( 'afl_actions_nonce', 'nonce' );

	$target = isset( $_POST['target'] ) ? (int) $_POST['target'] : 0;
	if ( ! $target ) wp_send_json_error( [ 'msg' => 'Missing target' ], 400 );
 ...
 * AJAX: Favorite
 */
add_action( 'wp_ajax_afl_toggle_favorite', function () {
	if ( ! is_user_logged_in() ) wp_send_json_error( [ 'msg' => 'Not logged in' ], 403 );
	check_ajax_referer( 'afl_actions_nonce', 'nonce' );
...
/**
 * Backup: update last active timestamp for logged-in users on init
 */
...
function all_profile_save_data( $user_id, $data ) {
	...
	if ( ! empty( $data['remove_gallery'] ) && is_array( $data['remove_gallery'] ) ) {
		...
		foreach ( $remove_ids as $rid ) {
			if ( $rid ) wp_delete_attachment( $rid, true );
		}
	...
	if ( isset( $_FILES['gallery_photos'] ) && ! empty( $_FILES['gallery_photos']['name'][0] ) ) {
		...
		for ( $i = 0; $i < count( $files['name'] ); $i++ ) {
			...
			if ( count( $existing_gallery ) >= 4 ) break;
			...
			if ( ! isset( $uploaded['file'] ) ) continue;
...
function all_profile_builder_shortcode() {
	...
	if ( isset( $_POST['all_profile_nonce'] ) && wp_verify_nonce( $_POST['all_profile_nonce'], 'all_save_profile' ) ) {
 ...
 * ✅ Camera icon opens lightbox (main + gallery) like your sample.
 */
function all_member_grid_shortcode() {
	...
	<style>
	  ...
	  .afl-icon-btn{
		...
		background:#f3f4f6;text-decoration:none;border:1px solid #e5e7eb;
	  }
	  ...
	  .afl-photo-count{
		...
		display:inline-flex;align-items:center;justify-content:center;
		background:#111827;color:#fff;font-size:10px;padding:0 5px;
	  }
	...
	<div class="afl-members-grid-wrapper">
	  ...
	  <div class="afl-members-grid">
		<?php foreach ( $users as $u ) :
			$uid     = (int) $u->ID;
			$is_self = ( get_current_user_id() === $uid );

			// main photo (card uses medium, lightbox uses full)
			...
			$lightbox_urls = [];
			if ( $photo_full ) $lightbox_urls[] = $photo_full;

			foreach ( $gallery_ids as $gid ) {
				$gid = (int) $gid;
				if ( ! $gid ) continue;
				$full = wp_get_attachment_image_url( $gid, 'full' );
				if ( $full ) $lightbox_urls[] = $full;
			}
			...
			$city     = get_user_meta( $uid, 'all_city', true );
			$country  = get_user_meta( $uid, 'all_country', true );
			...
			$age_city_country = trim(
				( $age ? intval( $age ) : '' ) .
				( $age && ( $city || $country ) ? ', ' : '' ) .
				trim( $city . ( $city && $country ? ', ' : '' ) . $country )
			);
			...
			<div class="afl-member-body">
			  ...
			  <div class="afl-member-seeking">
				...
				<span class="afl-seek-value">
				  ...
				  <?php if ( $seek_age ) echo ', ' . esc_html( $seek_age ); ?>
				</span>
			  </div>
			  ...
			  <!-- Hidden lightbox items (main + gallery) so arrows work -->
 ...
 * Shows one member’s full profile based on ?member=ID
 */
function all_single_member_profile_shortcode( $atts ) {
	...
	if ( ! $member_id && ! empty( $atts['member'] ) ) $member_id = intval( $atts['member'] );
	if ( ! $member_id ) $member_id = get_current_user_id();

	$user = get_user_by( 'id', $member_id );
	if ( ! $user ) return '<p>Member not found.</p>';
	...
	<div class="afl-profile-view-wrapper">
		...
		<div class="afl-profile-view-section">
			...
			<div class="afl-two-col">
				...
				<div><span class="afl-field-label">Preferred Age Range</span><span class="afl-field-value"><?php
					if ( $partner_age_min || $partner_age_max ) echo esc_html( $partner_age_min ) . ' - ' . esc_html( $partner_age_max );
				?></span></div>
